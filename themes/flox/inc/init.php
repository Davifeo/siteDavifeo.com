...
class Theme_Includes
{
	...
	public static function init()
	{
		 ...
		 */
		{
			...
			self::include_child_first('/ckav-elements/ckav-elements.php');
			self::include_child_first('/ckav-elements/ckav-fn.php');
		...
		if (is_admin()) {
			add_action('admin_enqueue_scripts', array(__CLASS__, '_action_ckavadmin_enqueue_scripts'),
				20 // Include later to be able to make wp_dequeue_style|script()
			);
		}
	}
	...
	public static function _action_ckavadmin_enqueue_scripts()
	{
		self::include_child_first('/admin_static.php');
	}
	...
	public static function _action_widgets_init()
	{
		...
		foreach ($paths as $path) {
			...
			foreach ($dirs as $dir) {
				...
				$widget_class = 'Ckav_' . self::dirname_to_classname( $dirname );
